...
<div class="space-y-5">
    ...
    <form method="GET" action="{{ route('user.notes.index') }}" class="flex flex-col md:flex-row gap-2.5 w-full xl:w-auto">
        ...
        <div class="flex gap-2">
            <select name="folder" id="folderSelect"
                ...
                @foreach($folders as $folder)
                    <option value="{{ $folder->id }}" data-name="{{ $folder->name }}" @selected(request('folder') == $folder->id)>
                        {{ $folder->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex gap-2">
            <select name="category" id="categorySelect"
                ...
                @foreach($categories as $category)
                    <option value="{{ $category->id }}" data-name="{{ $category->name }}" @selected(request('category') == $category->id)>
                        {{ $category->name }}
                    </option>
                @endforeach
            </select>
                ...
                <div
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="flex flex-wrap gap-2 mt-4">
                            @if($note->folder)
                                <span class="px-2.5 py-1 rounded-lg text-[12px] font-semibold dark:bg-white/[0.06] bg-gray-100 dark:text-gray-300 text-gray-600">
                                    📁 {{ $note->folder->name }}
                                </span>
                            @endif

                            @if($note->category)
                                <span class="px-2.5 py-1 rounded-lg text-[12px] font-semibold dark:bg-pink-500/[0.12] bg-pink-50 text-pink-500">
                                    {{ $note->category->name }}
                                </span>
                            @endif
                        </div>
                        ...
                        @if($note->tags->count())
                            <div class="flex flex-wrap gap-2 mt-3">
                                @foreach($note->tags as $tag)
                                    <span class="px-2 py-1 rounded-lg text-[11px] font-semibold dark:bg-[#21192c] bg-orange-50 dark:text-gray-400 text-orange-600">
                                        #{{ $tag->name }}
                                    </span>
                                @endforeach
                            </div>
            ...
            @empty
                <div class="md:col-span-2 xl:col-span-3">
                    <div class="rounded-2xl p-10 text-center dark:bg-[#17141f] bg-white border dark:border-white/[0.08] border-orange-100">
                        ...
                        <p class="text-[14px] dark:text-gray-500 text-gray-500 mt-2">Start by creating your first note.</p>
