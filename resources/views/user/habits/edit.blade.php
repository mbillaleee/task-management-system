...
@section('user')
    <section class="space-y-5">
        ...
        <div class="flex items-end justify-between gap-3">
            <div>
                ...
                <p class="text-[14px] dark:text-gray-500 text-gray-400 mt-0.5">
                    Update habit category, type, frequency and reminder settings.
                </p>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('user.habits.show', $habit) }}"
                    ...
                    <i class="fas fa-eye mr-1"></i> View Habit
                </a>
        ...
        <div class="dark:bg-[#17141f] bg-white border dark:border-white/[0.07] border-black/[0.07] rounded-2xl p-4">
            ...
            {{-- Existing categories + edit/delete --}}
            @if ($categories->count())
                <div class="flex flex-wrap gap-1.5 mt-3 pt-3 border-t dark:border-white/[0.06] border-black/[0.05]">
                    @foreach ($categories as $cat)
                        <div
                            class="group flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[12px]
                            ...
                            {{-- Delete button --}}
                            <form action="{{ route('user.habit-categories.destroy', $cat) }}" method="POST"
                                class="inline ml-1">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Delete this category?')"
                                    class="text-red-400 hover:text-red-500 text-[10px] leading-none">
