...
<section class="space-y-5">
    ...
    {{-- ── Page Header ── --}}
    <div>
        <h2 class="text-[20px] font-extrabold tracking-[-0.3px] dark:text-white text-gray-900">Settings</h2>
        <p class="text-[14px] dark:text-gray-500 text-gray-400 mt-0.5">Manage your account, appearance, and preferences.</p>
    </div>
    ...
    @if(session('success'))
        ...
        <script>setTimeout(()=>{ const a=document.getElementById('settingsAlert'); if(a) a.style.display='none'; }, 3500);</script>
    @endif
    @if(session('error') || $errors->any())
        <div class="px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-[13px] font-bold flex items-center gap-2">
    ...
    {{-- ── Profile Hero Card ── --}}
    <div class="relative overflow-hidden dark:bg-[#17141f] bg-white border dark:border-white/[0.07] border-black/[0.07] rounded-2xl p-5 flex flex-col sm:flex-row items-center sm:items-start gap-5">
        ...
        <div class="relative flex-shrink-0 z-10">
            <div class="w-20 h-20 rounded-2xl p-[2.5px]"
                ...
                <div class="w-full h-full rounded-[14px] overflow-hidden dark:bg-[#1a1625] bg-gray-100 flex items-center justify-center">
                    @if($user->profile)
            ...
            {{-- Online dot --}}
            <span class="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-emerald-400 border-2 dark:border-[#17141f] border-white"></span>
        </div>
        ...
        <div class="flex-1 z-10 text-center sm:text-left">
            ...
            <div class="flex flex-wrap items-center gap-2 mt-3 justify-center sm:justify-start">
                ...
                @else
                    <span class="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-gray-500/10 dark:text-gray-400 text-gray-500">
                        Free Plan
                    </span>
                @endif
                @if($user->username)
                    <span class="px-2.5 py-1 rounded-lg text-[11px] font-bold dark:bg-white/[0.06] bg-gray-100 dark:text-gray-400 text-gray-500">
                        @{{ $user->username }}
                    </span>
                @endif
                @if($user->country)
                    <span class="px-2.5 py-1 rounded-lg text-[11px] dark:bg-white/[0.06] bg-gray-100 dark:text-gray-400 text-gray-500">
                        📍 {{ $user->country }}
                    </span>
                @endif
                <span class="px-2.5 py-1 rounded-lg text-[11px] dark:bg-white/[0.06] bg-gray-100 dark:text-gray-400 text-gray-500">
    ...
    <div class="dark:bg-[#17141f] bg-white border dark:border-white/[0.07] border-black/[0.07] rounded-2xl overflow-hidden">
        ...
        <div class="p-5 sm:p-6">
            ...
            {{-- ══════════ NOTIFICATIONS TAB ══════════ --}}
            <div id="tab-notifications" class="tab-panel {{ $activeTab !== 'notifications' ? 'hidden' : '' }} space-y-6">
