...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div>
        <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">Plan Name <span
    ...
    <div>
        <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">Slug <span
                class="text-red-800">*</span></label>
...
{{-- ── Description ── --}}
<div>
    <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">Description</label>
...
<div class="dark:bg-[#1a1625] bg-gray-50 rounded-xl p-4 border dark:border-white/[0.07] border-black/[0.06] space-y-3">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div>
            <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">Monthly Price ($) <span
                    class="text-red-800">*</span></label>
            <input type="number" name="price_monthly" id="{{ $pfx }}price_monthly" value="0"
        ...
        <div>
            <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">Yearly Price ($) <span
                    class="text-red-800">*</span></label>
            <input type="number" name="price_yearly" id="{{ $pfx }}price_yearly" value="0" min="0"
                step="0.01" required
                class="w-full px-3.5 py-2.5 rounded-[10px] text-[14px] outline-none dark:bg-[#100e1a] bg-white dark:text-white text-gray-800 dark:border dark:border-white/[0.1] border border-black/[0.1] focus:border-orange-400 transition">
        </div>
        <div>
            <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">Currency</label>
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
    <div>
        <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">Icon (emoji)</label>
    ...
    <div>
        <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">Accent Color</label>
    ...
    <div>
        <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">Badge Label</label>
    ...
    <div>
        <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">Badge Color</label>
...
{{-- ── Feature Limits ── --}}
<div class="dark:bg-[#1a1625] bg-gray-50 rounded-xl p-4 border dark:border-white/[0.07] border-black/[0.06] space-y-3">
    <p class="text-[12px] font-bold dark:text-gray-400 text-gray-500 uppercase tracking-wide">Feature Limits <span
            class="dark:text-gray-600 text-gray-400 normal-case font-normal ml-1">(-1 = Unlimited)</span></p>
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        @foreach ($limits as $field => $label)
            <div>
                <label
                    class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">{{ $label }}</label>
                <input type="number" name="{{ $field }}" id="{{ $pfx }}{{ $field }}"
                    value="-1" min="-1"
                    class="w-full px-3.5 py-2.5 rounded-[10px] text-[14px] outline-none dark:bg-[#100e1a] bg-white dark:text-white text-gray-800 dark:border dark:border-white/[0.1] border border-black/[0.1] focus:border-orange-400 transition">
            </div>
        @endforeach
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
        ...
        @foreach ($flags as $field => $meta)
            <label class="flex items-center gap-2 cursor-pointer group">
                <input type="checkbox" name="{{ $field }}" id="{{ $pfx }}{{ $field }}"
                    value="1" class="w-4 h-4 accent-orange-500 rounded cursor-pointer">
                <span
                    class="text-[13px] font-bold dark:text-gray-600 text-gray-700 group-hover:dark:text-white group-hover:text-gray-900 transition flex items-center gap-1.5">
...
<div>
    <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">
        Custom Features List
        <span class="dark:text-gray-600 text-gray-800 font-normal ml-1">(one per line, shown on pricing page)</span>
    </label>
    <textarea name="features" id="{{ $pfx }}features" rows="4"
...
<div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
    <div>
        <label class="block text-[12px] font-bold dark:text-gray-600 text-gray-700 mb-1.5">Sort Order</label>
    ...
    <div class="flex flex-col justify-end gap-2 pb-0.5">
        <label class="flex items-center gap-2.5 cursor-pointer">
            <input type="checkbox" name="is_active" id="{{ $pfx }}is_active" value="1" checked
                class="w-4 h-4 accent-orange-500 rounded">
            <span class="text-[13px] font-bold dark:text-gray-600 text-gray-700">Active</span>
        </label>
        <label class="flex items-center gap-2.5 cursor-pointer">
            <input type="checkbox" name="is_featured" id="{{ $pfx }}is_featured" value="1"
                class="w-4 h-4 accent-orange-500 rounded">
            <span class="text-[13px] font-bold dark:text-gray-600 text-gray-700">Featured / Highlighted</span>
        </label>
    </div>
</div>
